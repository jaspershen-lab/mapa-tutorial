...
::: callout-important
Prerequisites: Before running LLM interpretation, ensure you have completed functional module generation using either SimCluster (`merge_modules()`) or EmbedCluster (`merge_pathways_bioembedsim()`) as described in previous chapters. The `enriched_functional_modules` object used in this chapter should be the output from the module clustering step.

```{r}
#| eval: false
# If you haven't performed module clustering yet, run:
# enriched_functional_modules <- merge_modules(...)
# or 
# enriched_functional_modules <- merge_pathways_bioembedsim(...)

# Load your clustering results if saved previously:
# load("path/to/your/enriched_functional_modules.RData")
```
:::
...
- Assigns confidence scores (0-1) reflecting the LLM's assessment of interpretation quality based on available evidence. High confidence menas strong literature support and clear biological coherence
...
Continue to [Data Visualization](06_data_vis.qmd) to create informative plots showcasing your LLM-interpreted modules.
