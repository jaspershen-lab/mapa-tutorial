...
-   A `functional_module` object from previous steps (`merge_pathways()` + `merge_modules()` **or** `merge_pathways_bioembedsim()`, optionally `llm_interpret_module()` if you want LLM summaries).

-   An `expression_data` table with an `id` column (gene/protein ENSEMBL; metabolite HMDB/KEGG) followed by one column per sample.
:::
