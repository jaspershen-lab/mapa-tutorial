...
### Gene Expression Data for *GSEA*
  ...
  1. **One identifier column**  
     ...
     If the user uploads other types of IDs, the `entrezid` will be matched by the first step of MAPA:  
     ```r
     convert_id()
     ```
  ...
  2. **An `order_by` column**  
     ...
     - For MAPA R package users: the column name should be one of `fc` or `p_value_adjust`.  
     - There are many ranking metrics—provide user guidance.  
       Reference:  
       Zyla, J., Marczyk, M., Weiner, J. et al. *Ranking metrics in gene set enrichment analysis: do they matter?*  
       **BMC Bioinformatics** 18, 256 (2017). [https://doi.org/10.1186/s12859-017-1674-0](https://doi.org/10.1186/s12859-017-1674-0)  
     - For MAPA R package users: there is no strict requirement for the column name,  
       but the `order_by` parameter should be set to the column name of the ranking metrics.  
